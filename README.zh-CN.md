...
- **只读 Subagent** —— 委派隔离的工作区调研任务，并行处理相互独立的问题。
- **长会话友好** —— 自动上下文压缩。
...
每个新 turn 都会由共享 runtime 把解析后的模型调用信息写入记录。Web、CLI、Desktop、WSL 与远端工作区的实时执行步骤和恢复后的会话历史因此统一使用同一个模型名称。
...
支持工具调用的模型会自动获得 `delegate_task` 工具，用于执行自包含的工作区调研任务。模型可以在同一次回复中发出多个 `delegate_task` 调用，并行调查相互独立的问题；父 turn 会等待全部结果后继续执行。

Subagent 继承当前选择的模型、推理档位和已配置的 system prompt，但使用隔离对话，只接收委派任务。它只能调用 `read_file`、`list_files` 和 `search_text`，不能修改文件、运行 shell、调用 MCP 工具或继续派生 subagent。

每个父 turn 最多启动 4 个 subagent，最多并发运行 4 个；单任务超时为 5 分钟，返回报告最多保留 12,000 个字符。CLI、JSONL、Web、Desktop 与 WSL 都会显示 subagent 开始和结束事件。父会话只保存委派任务和最终工具结果，不保存 subagent 的完整思考过程与内部消息轨迹。

每次委派都会从全局子智能体名单中获得一个稳定身份。默认名单包含 22 个内置成员；现在可以在 **设置 → 子智能体** 中改名、新增、删除、恢复默认名单，并为每个姓名上传头像。名单保存在 `~/.morrow/subagents.json`，Unix 上文件权限为 `0600`；名单必须保留 4–64 个不重名成员。变更从下一个 turn 生效，已经运行的 turn 继续使用启动时快照。CLI、Web、Desktop、WSL 与远端 turn 都使用同一个身份池；Desktop 的 WSL 模式由 Windows 主机保存名单和头像。

同一父 turn 内不会重复分配身份，后续 turn 可以复用。执行时的稳定 ID 和姓名会随最终工具结果持久化，但头像数据不会写入 Session，也不会发送到远端 runtime。因此历史标题保留执行时姓名，头像则跟随该 ID 当前配置；配置被删除后自动回退机器人图标。浏览器会先把 PNG、JPEG 或 WebP 原图居中裁切为 256×256，并优先编码为 WebP，服务端再次校验后才保存。仪表盘默认折叠每个 subagent 步骤，用户展开后才显示固定高度、提示词与输出分别滚动的双面板卡片；输出区在任务完成后展示最终 Markdown 报告，不流式展示 subagent 的内部轨迹。
