...
每次委派都会从 Morrow 内置名单中获得一个显示姓名；同一父 turn 内不会重名，后续 turn 可以复用。姓名会随最终工具结果持久化，因此实时事件与历史记录保持一致。仪表盘默认折叠每个 subagent 步骤，用户展开后才显示固定高度、提示词与输出分别滚动的双面板卡片；输出区在任务完成后展示最终 Markdown 报告，不流式展示 subagent 的内部轨迹。
